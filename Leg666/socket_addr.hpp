...
public:
    SocketAddr(const char* ip="", int port=8080, bool is_v4=true){
        if(is_v4){
            _is_v4 = true;
    ...
    sockaddr* get_addr(){
        return _is_v4 ? (sockaddr*)&_addr_v4 : (sockaddr*)&_addr_v6;
    }

    socklen_t get_addr_len(){
        return _is_v4 ? sizeof _addr_v4 : sizeof _addr_v6;
    }
    

private:
    bool         _is_v4;    // 是否是 IPv4 地址
    sockaddr_in  _addr_v4;  // IPv4 地址
    sockaddr_in6 _addr_v6;  // IPv6 地址
};
}   // namespace jiao